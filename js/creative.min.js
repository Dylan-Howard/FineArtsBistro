...
window.sr=ScrollReveal();sr.reveal('.sr-icons',{duration:600,scale:0.3,distance:'0px'},200);sr.reveal('.sr-button',{duration:1000,delay:200});sr.reveal('.sr-contact',{duration:600,scale:0.3,distance:'0px'},300);$('.popup-gallery').magnificPopup({delegate:'a',type:'image',tLoading:'Loading image #%curr%...',mainClass:'mfp-img-mobile',gallery:{enabled:!0,navigateByImgClick:!0,preload:[0,1]},image:{tError:'<a href="%url%">The image #%curr%</a> could not be loaded.'}});if(($(window).width()<768)||(($(window).width()<1028)&&($(window).height()>1028))){$('header').css('background-image','url(\'./img/bg/mob/1.jpg\')');$('.section-heading').css('background-image','url(\'./img/bg/mob/2.jpg\')')}
window.onscroll=function(){$('.parallax-head').css('background-position','50% '+((window.pageYOffset*0.2))+'px');$('#our-team').css('background-position','50% '+((window.pageYOffset-$('#our-team').position().top)*0.2)+'px');$('#visit').css('background-position','50% '+((window.pageYOffset-$('#visit').position().top)*0.2)+'px')}})(jQuery)
